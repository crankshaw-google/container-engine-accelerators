...
def find_schedulable_pods_ray_cluster(pods, gate_name):
  ...
  # Note: group by ray_cluster, ray_worker_group then assign consecutive indices
...
def schedule_pod_on_node(v1, pod_name, pod_namespace, node_name, gate_name, index):
  ...
  try:
    ...
    if any(gate.name == gate_name for gate in pod.spec.scheduling_gates):
      ...
      pod.metadata.labels["ray_group_index"] = str(index)
      if 'ray.io/group' in pod.metadata.labels:
        pod.metadata.labels["ray_group"] = pod.metadata.labels['ray.io/group']
...
def compute_assignment_distance(sorted_nodes, sorted_pods, assignment, running_pods):
  ...
  used_nodes = [sorted_nodes[a] for a in assignment]

  if running_pods:
    nodes_dict = {node["name"]: node for node in sorted_nodes}
    used_nodes += [nodes_dict[p.spec.node_name] for p in running_pods]
    used_nodes = sorted(used_nodes, key=node_topology_key)

  assert len(used_nodes) == len(sorted_pods) + len(running_pods)

  new_distance = 0
  for i in range(1, len(used_nodes)):
    new_distance += node_topology_distance(
        used_nodes[i], used_nodes[i - 1]
    )
  return new_distance
...
def calculate_pods_assignment(sorted_nodes, sorted_pods, running_pods):
  ...
  while True:
    ...
    while i >= 0 and all_ok:
      ...
      # elif i < len(assignment) - 1 and assignment[i] != assignment[i + 1]:
        all_ok = False
    if assignment[-1] == len(sorted_nodes):
      break
    if all_ok:
      new_distance = compute_assignment_distance(
          sorted_nodes,
          sorted_pods,
          assignment,
          running_pods)
...
def schedule_pod_with_gate(v1, gate, ignored_namespace):
  ...
  for job in sorted_jobs:
    ...
    for i in range(0, len(sorted_pods)):
      ...
      schedule_pod_on_node(
          v1, pod['name'], pod['namespace'], node['name'], gate, pod['index']
      )
    time.sleep(1)


def list_pods(v1, ignored_namespace):
